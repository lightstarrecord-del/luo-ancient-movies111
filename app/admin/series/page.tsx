...
export default function AdminSeriesPage() {
  ...
  const [form, setForm] = useState({ title: "", image: "", description: "", playLink: "", downloadLink: "" });
  ...
  const handleSubmit = async (e: any) => {
    ...
    setForm({ title: "", image: "", description: "", playLink: "", downloadLink: "" });
  };
  const handleEdit = (item: any) => {
    setEditId(item.id);
    setForm({ title: item.title, image: item.image, description: item.description || "", playLink: item.playLink || "", downloadLink: item.downloadLink || "" });
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-black text-white min-h-screen">
      ...
      <form onSubmit={handleSubmit} className="mb-8 space-y-4 bg-gray-900 p-4 rounded">
        ...
        {editId && <button type="button" onClick={() => { setEditId(null); setForm({ title: "", image: "", description: "", playLink: "", downloadLink: "" }); }} className="w-full mt-2 bg-gray-700 text-white py-2 rounded">Cancel Edit</button>}
      </form>
